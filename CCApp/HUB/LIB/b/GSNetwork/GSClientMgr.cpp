		...
		if (client.second->GetConnected())
		{
			...
			if (server_check_time != 0 && client_time != 0 && (client_time + server_check_time) <= system_tick)
			{
				if (client.second->GetClientType() != ClientType::SERVER_CLIENT)
				{
					...
					//else if(client.second->m_TryCloseCount > 1)//가끔 가다가 위에서 종료를 시켜도 안되는 애들은 여기서 그냥 보낸다. alive time을 너무 길게 잡아서 그런거 같다 하지만 보험으로 하나 들어온다 밑에 코드는 들어온적은 없다.
					//{
					//	byte arry[2];
					//	client.second->GetTCPSocket()->Write(arry, 1);
					//}
...
int GSClientMgr::PopRecycleId()
{
	...
	//만번부터 
	if (m_Reycle_Id_Queue.unsafe_size() < 100000)
		return -1;
...
BOOL GSClientMgr::NewClient(SOCKET DisConnectSock)
{
	GSServer::GSServer *pServer = (GSServer::GSServer *)m_GSServer;
	...
	for (DWORD i = 0; i < NewClient; i++)
	{
		...
		if (!pClient->GetTCPSocket()->AcceptReUseSocket(ListenSocket, DisConnectSock))
		{
		//	End();
			return FALSE;
		}
