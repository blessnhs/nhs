...
unsigned __stdcall CreateOutofMemoryLog( void* pArguments )
{
    ...
    const auto&& path = CrashHandler::GenerateDumpFilePath();
    OutdatedStackwalker walker(path.second);
...
void OutdatedStackwalker::OnOutput(LPCSTR inText)
{
    /*
    if (boost::find_last(inText, ".h") ||
    boost::find_last(inText, ".hpp") ||
    boost::find_last(inText, ".cpp") ||
    boost::find_last(inText, ".c") ||
    boost::find_last(inText, ".cc")))
    {
    _stream << "\n\n\n";
    }*/
