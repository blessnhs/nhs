...
//이락은 필수 안그럼 종료되었는데 남아 있고 꼬인다.
	...
	if (pClient == NULL)
	{
		SYSLOG().Write("!!!connect alert cant find onconnect client id %d\n",client_id);
		return;
	}
	...
	{
		SYSLOG().Write("Connected exception ...1  %d\n", pClient->GetTCPSocket()->GetSocket());
	...
	if (!pClient->InitializeReadForIocp())
	{ 
		SYSLOG().Write("Connected exception ...2 %d\n", pClient->GetTCPSocket()->GetSocket());
		//이코드에서 그냥 close하면 gsclient는 서버에 접속도 못하는 미아 객체가 됨
	...
	LPFN_DISCONNECTEX lpfn_disconnect = (LPFN_DISCONNECTEX)DisconnectEx((const SOCKET)pClient->GetTCPSocket()->GetSocket(), WSAID_DISCONNECTEX);
...
VOID GSServer::OnDisconnected(int client_id, bool isForce)
{
	...
	GetClientMgr().NewClient(pClient->GetTCPSocket()->GetSocket());
...
BOOL GSServer::BeginTCP()
{
	...
	BOOL on = TRUE;
	setsockopt(m_pTCPListen->GetSocket(), SOL_SOCKET, SO_CONDITIONAL_ACCEPT,
		reinterpret_cast<char*>(&on), sizeof(on));

	if (!m_pTCPListen->GetTCPSocket()->Listen((WORD)m_Arguments.m_BindPort, SOMAXCONN))
	{
		GSServer::End();
