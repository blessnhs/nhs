 ...
 *  TCB를 해제함
 */
static void FreeMemoryTCB( QWORD qwID )
{
    int i;
                  ...
                  QWORD qwEntryPointAddress, BYTE bAffinity )
{
    ...
    // 동적 메모리 영역에서 스택 할당
    pvStackAddress = AllocateMemory( TASK_STACKSIZE );
    if( pvStackAddress == NULL )
    {
        FreeMemoryTCB( pstTask->stLink.qwID );
        return NULL;
    }
    ...
    SetUpTask( pstTask, qwFlags, qwEntryPointAddress, pvStackAddress, 
            TASK_STACKSIZE );
                 ...
                 void* pvStackAddress, QWORD qwStackSize )
{
    ...
    // Return Address 영역에 ExitTask() 함수의 어드레스를 삽입하여 태스크의 엔트리
    // 포인트 함수를 빠져나감과 동시에 ExitTask() 함수로 이동하도록 함
...
static TCB* GetNextTaskToRun( BYTE bAPICID )
{
    ...
    for( j = 0 ; j < 2 ; j++ )
    {
        ...
        for( i = 0 ; i < TASK_MAXREADYLISTCOUNT ; i++ )
        {
            iTaskCount = GetListCount( &( gs_vstScheduler[ bAPICID ].
                    vstReadyList[ i ] ) );
            ...
            {
                pstTarget = ( TCB* ) RemoveListFromHeader( 
...
static BOOL AddTaskToReadyList( BYTE bAPICID, TCB* pstTask )
{
    ...
    {
        AddListToTail( &( gs_vstScheduler[ bAPICID ].stWaitList ), pstTask );
        return TRUE;
    }
    ...
    AddListToTail( &( gs_vstScheduler[ bAPICID ].vstReadyList[ bPriority ] ), 
            pstTask );
    return TRUE;
}
...
static TCB* RemoveTaskFromReadyList( BYTE bAPICID, QWORD qwTaskID )
{
    ...
    pstTarget = RemoveList( &( gs_vstScheduler[ bAPICID ].vstReadyList[ bPriority ]), 
                     qwTaskID );
    return pstTarget;
}
...
int GetReadyTaskCount( BYTE bAPICID )
{
    ...
    for( i = 0 ; i < TASK_MAXREADYLISTCOUNT ; i++ )
    {
        iTotalCount += GetListCount( &( gs_vstScheduler[ bAPICID ].
                vstReadyList[ i ] ) );
    }
...
static BYTE FindSchedulerOfMinumumTaskCount( const TCB* pstTask )
{
    ...
    iCurrentTaskCount = GetListCount( &( gs_vstScheduler[ pstTask->bAPICID ].
    ...
    for( i = 0 ; i < iProcessorCount ; i++ )
    {
        ...
        // 모든 스케줄러를 돌면서 확인
        iTempTaskCount = GetListCount( &( gs_vstScheduler[ i ].vstReadyList[ 
            bPriority ] ) );
...
void IdleTask( void )
{
    ...
    while( 1 )
    {
        ...
        if( GetListCount( &( gs_vstScheduler[ bCurrentAPICID ].stWaitList ) ) 
                > 0 )
        {
            while( 1 )
            {
                ...
                if( pstTask->qwFlags & TASK_FLAGS_PROCESS )
                {
                    ...
                    else
                    {
                        ...
                        {
                            FreeMemory( pstTask->pvMemoryAddress );
                        }
                    }
                }                
                    ...
                    if( pstProcess != NULL )
                    {
                                ...
                                &bProcessAPICID ) == TRUE )
                        {
                            RemoveList( &( pstProcess->stChildThreadList ), 
                                         pstTask->stLink.qwID );
                            UnlockForSpinLock( &( gs_vstScheduler[ 
                ...
                // 스택을 반환
                FreeMemory( pstTask->pvStackAddress );
                 
                // 태스크 자료구조(TCB)를 반환
                FreeMemoryTCB( qwTaskID );                
