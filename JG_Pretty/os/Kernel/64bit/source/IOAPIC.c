    ...
    for( i = 0 ; i < pstMPHeader->wEntryCount ; i++ )
    {
        ...
        case MP_ENTRYTYPE_IOINTERRUPTASSIGNMENT:
                ...
                ( pstIOAssignmentEntry->bInterruptType == MP_INTERRUPTTYPE_INT ) )                        
            {
                ...
                // 이용하여 처리
                WriteIOAPICRedirectionTable( pstIOAssignmentEntry->bDestinationIOAPICINTIN, 
...
void RoutingIRQToAPICID( int iIRQ, BYTE bAPICID )
{
    ...
    ReadIOAPICRedirectionTable( gs_stIOAPICManager.vbIRQToINTINMap[ iIRQ ],
            &stEntry );
    stEntry.bDestination = bAPICID;
    WriteIOAPICRedirectionTable( gs_stIOAPICManager.vbIRQToINTINMap[ iIRQ ],
            &stEntry );
}
