...
BOOL InitializeHDD(void)
{
    ...
    // 하드 디스크 정보 요청
    if( ReadHDDInformation( TRUE, TRUE, &( gs_stHDDManager.stHDDInformation ) ) 
            == FALSE )
    {
...
BOOL ReadHDDInformation( BOOL bPrimary, BOOL bMaster, HDDINFORMATION* pstHDDInformation )
{
    ...
    // 문자열은 바이트 순서로 다시 변환
    SwapByteInWord( pstHDDInformation->vwModelNumber,
            sizeof( pstHDDInformation->vwModelNumber ) / 2 );
    SwapByteInWord( pstHDDInformation->vwSerialNumber,
...
BOOL ReadUSBInformation( BOOL bPrimary, BOOL bMaster, HDDINFORMATION* pstHDDInformation )
{
	 WORD wPortBase;
	 QWORD qwLastTickCount;
	 BYTE bStatus;
